...
# import packages
import os
from bokeh.models import ColumnDataSource, CategoricalColorMapper, DataRange1d, ColorBar, Select, Button, Slider, Legend, RadioGroup, BoxAnnotation, Label
...
class bokeh_layout:
    ...
    def __init__(self, profile_data, pump_data, output_path, downcast_data):
        ...
        self.next_profile = Button(label=">", 
                                   button_type='primary', width=50, 
                                   disabled=False,
                                   )
        # Previous profile allows users to click through to the previous cast of the cruise 
        self.prev_profile = Button(label="<",
                                   button_type='default',
                                   width=50,
                                   disabled=True,
                                   )
        self.o2adv1 = Slider(title="Primary oxygen sensor alignment [s]",
                             start=-12,
                             end=12,
                             step=1,
                             value=int(self.oxy_align_default),
                             )
        self.o2adv2 = Slider(title="Secondary oxygen sensor alignment [s]",
                             start=-12,
                             end=12,
                             step=1,
                             value=int(self.oxy_align_default),
        ...
        self.cast_start = Button(label="Set cast start",
                                 button_type='primary',
                                 width=200,
                                 disabled=False,
                                 )
    ...
    def surface_soak(self):
        ...
        pump_time_on = self.pump_data[self.pump_data['profile']==self.profile.value].copy(deep=True)
        ...
        pump_color_mapper = CategoricalColorMapper(factors = pumps_factors,
                                              palette = pumps_palette,
                                             )
        pump_color_dict = dict(field='pumps_txt', transform = pump_color_mapper)
        
        # Set up plots
        surface_soak_plot = figure(width=500,
                    height=500,
               x_axis_label='Cycles',
               y_axis_label='prDM',
        ...
        # Add data to plot
        surface_soak_plot.scatter('Cycles', 'prDM',
        ...
        if 'sbeox0Mm/L' in self.param_list:
            ...
            surface_soak_oxy_temp_plot = figure(width=500,
                                                height=500,
            ...
            # Add data to plot
            surface_soak_oxy_temp_plot.line('sbeox0Mm/L', 't090C',
        ...
        if 'sbeox1Mm/L' in self.param_list:
            # Set up oxygen vs temperature plot settings
            surface_soak_oxy_temp2_plot = figure(width=500,
                                                 height=500,
            ...
            # Add data to plot
            surface_soak_oxy_temp2_plot.line('sbeox1Mm/L', 't190C',
    ...
    def update_surface_soak_plot(self, attr, old, new):
        ...
        pump_time_updated = self.pump_data[self.pump_data['profile']==self.profile.value].copy(deep=True)
    ...
    def update_cast_start(self):
        ...
        if len(point)!=1:
            ...
            self.pump_data.loc[self.pump_data['profile']==self.profile.value,'Cycles'] = icycles
            self.pump_data.loc[self.pump_data['profile']==self.profile.value,'timeS'] = itimeS
            self.pump_data.loc[self.pump_data['profile']==self.profile.value,'prDM'] = iprDM
            ...
            #update plot for pump dataframe source
            self.pump_on_time_updated=self.pump_data[self.pump_data['profile']==self.profile.value].copy(deep=True)
    ...
    def surface_soak_screening(self, doc):
        ...
        if 'sbeox0Mm/L' in self.param_list:
            layout = column(row(Spacer(width=40), self.prev_profile, 
                                Spacer(width=50), self.profile, 
                                self.next_profile, Spacer(width=50), 
                                Spacer(width=50), self.o2adv1),
                            row(self.cast_start),
                            row(self.surface_soak_plot,
    ...
    def heave(self):
        ...
        heave_df = self.downcast_data[self.downcast_data['CTD number']==self.profile.value].copy(deep=True)
        ...
        plot_tooltips = [("velocity", "@CTDvel"),
                         ("QC", "@prDM_QC"),
                        ]

        # Set up pressure vs time plot settings
        pressure_time_plot = figure(width=500,
                    height=500,
                    x_axis_label='timeS',
                    y_axis_label='prDM',
                    x_axis_location="below",
        ...
        # Add data to plot
        pressure_time_plot.scatter('timeS', 'prDM',
        ...
        pressure_temperature_plot = figure(width=500,
                    height=500,
                    x_axis_label='t090C',
                    y_axis_label='prDM',
                    x_axis_location="below",
        ...
        # Add data to plot
        pressure_temperature_plot.scatter('t090C', 'prDM',
                      source = self.col_src_heave,
                      color=heave_color_dict,
                      fill_alpha=0.2,
                      size=5,
        ...
        pressure_salinity_plot = figure(width=500,
                    height=500,
                    x_axis_label='sal00',
                    y_axis_label='prDM',
                    x_axis_location="below",
        ...
        # Add data to plot
        pressure_salinity_plot.scatter('sal00', 'prDM',
                      source = self.col_src_heave,
                      color=heave_color_dict,
                      fill_alpha=0.2,
                      size=5,
    ...
    def update_heave_plot(self, attr, old, new):
        ...
        df_updated = self.downcast_data[self.downcast_data['CTD number']==self.profile.value].copy(deep=True)
    ...
    def heave_screening(self, doc):
        ...
        layout = column(row(Spacer(width=40), self.prev_profile, 
                            Spacer(width=50), self.profile, self.next_profile, 
                            Spacer(width=50)),
                        row(self.pressure_time_plot,
    ...
    def bin_data(self):
        ...
        self.screen_print = Button(label="Save as HTML",
                              button_type='default',
                              width=50,
                              disabled=False,
                             )
        self.sensor_suite = RadioGroup(labels=['primary','secondary'],
                                  active=0,
                                 )
        self.x_axis_filter = Select(options = ['Longitude [degrees_east]', 
        ...
        self.col_src_metadata = ColumnDataSource(self.profile_data[['CTD number','Eastings','Northings']].drop_duplicates())
        ...
        # Map plot
        survey_map = figure(tools="pan,wheel_zoom,box_zoom,box_select,tap,reset",
        ...
        depth_temperature = figure(x_axis_label='Temperature [degC]',
        ...
        # Add data to plot
        depth_temperature.scatter('temp', 'depth',
                      source = self.col_src_bin,
                      color='green',
                      fill_alpha=0.2,
                      size=5,
                     )
        depth_temperature.line('temp', 'depth',
                      source = self.col_src_bin,
                      color='green',
                      line_alpha=1,
                      line_width=0.5,
        ...
        depth_conductivity = figure(x_axis_label='Conductivity [S/m]',
                    y_axis_label='Depth [m]',
                    x_axis_location="above",
        ...
        # Add data to plot
        depth_conductivity.scatter('cond', 'depth',
                      source = self.col_src_bin,
                      color='green',
                      fill_alpha=0.2,
                      size=5,
                     )
        depth_conductivity.line('cond', 'depth',
                      source = self.col_src_bin,
                      color='green',
                      line_alpha=1,
                      line_width=0.5,
        ...
        depth_salinity = figure(x_axis_label='Salinity [dimensionless]',
                    y_axis_label='Depth [m]',
                    x_axis_location="above",
                    y_range=self.depth_temperature.y_range,
                    tools="pan,wheel_zoom,box_zoom,box_select,tap,reset",
        ...
        # Add data to plot
        depth_salinity.scatter('sal', 'depth',
                      source = self.col_src_bin,
                      color='green',
                      fill_alpha=0.2,
                      size=5,
                     )
        depth_salinity.line('sal', 'depth',
                      source = self.col_src_bin,
                      color='green',
                      line_alpha=1,
                      line_width=0.5,
        ...
        pressure_oxygen = figure(x_axis_label='Oxygen [umol/L]',
                    y_axis_label='Depth [m]',
                    x_axis_location="above",
                    y_range=self.depth_temperature.y_range,
                    tools="pan,wheel_zoom,box_zoom,box_select,tap,reset",
        ...
        # Add data to plot
        pressure_oxygen.scatter('oxy_conc', 'depth',
                      source = self.col_src_bin,
                      color='green',
                      fill_alpha=0.2,
                      size=5,
                     )
        pressure_oxygen.line('oxy_conc', 'depth',
                      source = self.col_src_bin,
                      color='green',
                      line_alpha=1,
                      line_width=1,
        ...
        pressure_sigma = figure(x_axis_label='SigmaTheta [kg/m^3]',
                    y_axis_label='Depth [m]',
                    x_axis_location="above",
                    y_range=self.depth_temperature.y_range,
                    tools="pan,wheel_zoom,box_zoom,box_select,tap,reset",
        ...
        # Add data to plot
        pressure_sigma.scatter('sigma-theta', 'depth',
                      source = self.col_src_bin,
                      color='green',
                      fill_alpha=0.2,
                      size=5,
                     )
        pressure_sigma.line('sigma-theta', 'depth',
                      source = self.col_src_bin,
                      color='green',
                      line_alpha=1,
                      line_width=1,
        ...
        for item in patch_dict.keys():
            ts_plot.patch(patch_dict[item]['patch_coords'][0], patch_dict[item]['patch_coords'][1], line_color='red', line_width=1, alpha=0.5, fill_color='red', fill_alpha=0.1, )
            ts_plot.add_layout(Label(x=patch_dict[item]['label_coords'][0], y=patch_dict[item]['label_coords'][1], x_units="data", y_units="data", text=patch_dict[item]['label'],text_align='center', text_baseline='middle'))
        ...
        temp_section = figure(tools="pan,wheel_zoom,box_zoom,box_select,tap,reset",
                              x_axis_location='above',
                              y_axis_label = 'Depth [m]',
                              y_range=DataRange1d(flipped=True),
                              output_backend='webgl',
        ...
        # Add data to plot
        temp_section.scatter('SectionX', 'depth',
                             source = self.col_src_ts,
        ...
        sal_section = figure(tools="pan,wheel_zoom,box_zoom,box_select,tap,reset",
                             x_axis_location='above',
                             y_axis_label = 'Depth [m]',
                             y_range=DataRange1d(flipped=True),
                             output_backend='webgl',
        ...
        # Add data to plot
        sal_section.scatter('SectionX', 'depth',
                             source = self.col_src_ts,
        ...
        doxy_section = figure(tools="pan,wheel_zoom,box_zoom,box_select,tap,reset",
                              x_axis_location='above',
                              y_axis_label = 'Depth [m]',
                              y_range=DataRange1d(flipped=True),
                              output_backend='webgl',
        ...
        # Add data to plot
        doxy_section.scatter('SectionX', 'depth',
                            source = self.col_src_ts,
    ...
    def bin_screen(self, doc):
        ...
        layout = column(row(Spacer(width=40), self.prev_profile, 
                            Spacer(width=50), self.profile, 
                            self.next_profile, Spacer(width=150),
                            self.sensor_suite, Spacer(width=50),
                            self.screen_print,
                           ),
                        row(gridplot([self.depth_temperature,
                        ...
                        row(Spacer(width=395),
                            gridplot([self.oxygen_sat,
                                      self.turbidity],
                                     ncols=2, width=230, height=450,),),
