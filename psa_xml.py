...
def main():
    ...
    for psa_file in psa_files_with_2_sensors:
        ...
        calc_array_tag = "CalcArray"
        if calc_array is None:
            calc_array = tree.find("AverageCalcArray")
            calc_array_tag = "AverageCalcArray"
        ...
        pretty_xml_str = "{{# data}}\n" + pretty_xml_str + "\n\n{{/ data}}\n\n"
        file_out = Path("./psa_templates/only_primary").joinpath(Path(psa_file).name)
